...
#install_keras()
#install.packages(c('keras','neuralnet'))
...
class(result$no_show)
class(result$predicted)

ttest <- t.test(result$predicted,result$no_show)
ttest

preds <- as.factor(result$predicted)
act <- as.factor(result$no_show)
