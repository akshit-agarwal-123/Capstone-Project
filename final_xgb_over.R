...
#install.packages(c('xgboost','magrittr','dplyr','Matrix','ROSE'))
#install.packages(c('lattice','ggplot2','caret','car'))
...
print(t)
class(pred$label)
ttest <- t.test(as.numeric(pred$max_prob), as.numeric(pred$label))
ttest
cm <- confusionMatrix(as.factor(pred$max_prob),as.factor(pred$label))
